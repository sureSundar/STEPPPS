...
- `shell/shell.c` owns command semantics; other shells become frontends or
  ...
  purpose, not fall out silently from accepting this bullet. **CC resolution
  (2026-07-18)**: this decision has been made, on purpose, by Guru - not in
  favor of `shell/shell.c`. `make shell` was broken (see the row above), and
  the fix repoints it at `src/shell/tbos_tier_shell.c` (via `tier-shell-super`),
  the shell this entire session's live-testing has actually been built around.
  `shell/shell.c` remains an open question for the canonical manifest as a
  whole, but it is no longer what `make shell` produces or should produce.
