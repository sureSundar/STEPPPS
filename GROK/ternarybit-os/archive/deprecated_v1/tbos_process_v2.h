...
typedef struct tbos_process_manager_v2 {
    ...
    struct {
        ...
        tbos_process_v2_t** ready_queues[10]; // Multi-level priority queues
        tbos_process_v2_t* blocked_queue;
        tbos_process_v2_t* zombie_queue;
 ...
 */
typedef struct {
    tbos_process_v2_t* (*create_process)(const char* name, void (*entry)(void*), void* args);
...
tbos_process_manager_v2_t* tbos_process_manager_create(const char* scheduler_type);
void tbos_process_manager_destroy(tbos_process_manager_v2_t* manager);
...
extern tbos_process_manager_v2_t* g_tbos_process_manager_v2;
