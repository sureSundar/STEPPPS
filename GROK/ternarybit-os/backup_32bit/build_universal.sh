...
compile_c() {
    ...
    # Use -O0 for all kernel C files to avoid GCC optimization bugs and calling convention issues
    local opt_level="-O0"
    echo "    [$rel] Compiling with -O0"
...
assemble_bootloader_stage2() {
    ...
    nasm -I "$BOOT_DIR" -f bin \
        -D BOOT_KERNEL_LBA_START="$kernel_start" \
...
report_summary() {
    ...
    echo ""
    echo "Run with:"
    echo "  qemu-system-i386 -drive file=$OUTPUT_IMG,format=raw,if=floppy -m 512"
    echo ""
}
