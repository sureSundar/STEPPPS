...
// IDT entry structure for 32-bit protected mode
typedef struct {
    uint16_t offset_low;    // Offset bits 0-15
    uint16_t selector;      // Code segment selector
    uint8_t  zero;          // Always zero in 32-bit descriptor
    uint8_t  type_attr;     // Type and attributes
    uint16_t offset_high;   // Offset bits 16-31
} __attribute__((packed)) idt_entry_t;

// IDT pointer structure for 32-bit protected mode
typedef struct {
    uint16_t limit;
    uint32_t base;
...
// Set IDT gate for 32-bit protected mode
