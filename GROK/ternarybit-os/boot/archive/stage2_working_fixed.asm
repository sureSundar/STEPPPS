...
protected_mode_32:
    ...
    call show_sacred_menu

    ; Wait for user input and respond
    call wait_for_user_action

    ; After user interaction, continue or halt
    cli
.halt:
    hlt
    jmp .halt
...
.enter_key:
    call show_response
    mov esi, enter_response
    jmp .show_msg

.option1:
    call show_response
    mov esi, option1_response
    jmp .show_msg

.option2:
    call show_response
    mov esi, option2_response
    jmp .show_msg

.option3:
    call show_response
    mov esi, option3_response
    jmp .show_msg

.escape_key:
    call show_response
    mov esi, escape_response
    jmp .show_msg
...
alpine_complete_msg db 'Alpine GUI: Ready', 10, 0

; Interactive Menu Messages
menu_title db 'TBOS Sacred Interface', 10
           db 'Swamiye Saranam Aiyappa', 10, 0

menu_options db '1. STEPPPS Status', 10
             db '2. System Info', 10
             db '3. Meditation', 10
             db 'ENTER. Full Kernel', 10
             db 'ESC. Shutdown', 10, 0

input_prompt db 'Choose path (1-3, ENTER, ESC):', 0

enter_response db 'Loading Full Kernel...', 0
option1_response db 'STEPPPS: Active', 0
option2_response db 'TBOS v1.0 Ready', 0

option3_response db 'Om Namah Shivaya...', 0
escape_response db 'Sacred Shutdown', 0
